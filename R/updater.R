...
# loc <- cjeudb()
# cjeuUpload()
# # cjeu_sql(dbGet(loc, "SELECT `case` FROM Cases WHERE `case` LIKE '%/11%'")$case)
# # run <- dbGet(con, "SELECT `case` FROM cases WHERE `proceeding_number` IS NULL OR `proceeding_year` IS NULL")$case

# # Fix for example C-1321/79 first - proceeding shouldn't be null when joined cases is not found in source
# # cjeu_sql(c("C-1/55", "C-18/57")) # here be bugs
